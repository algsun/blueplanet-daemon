...
public class CRCUtil {
    static final int[] TABLE = {0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280,
     ...
     * 计算 crc ,但除过最后两个字节
     *
     * TODO 建议重命名为 genCRCWithoutTail @gaohui 2013-08-06
     *
     * @param data 包含crc的数据包
     */
    public static int getCRC(List<Byte> data) {
        return generateCRC(data.subList(0, data.size() - 2));
    }

    /**
     * 16位，根据数据包生成crc16(Modbus 算法)
     *
     * @param data 不包含crc的数据包
     * @return
     */
    public static int generateCRC(List<Byte> data) {
        int modbus_crc = 0xffff;
        Byte[] byteArray = data.toArray(new Byte[]{});
        for (Byte aByteArray : byteArray) {
            modbus_crc = (modbus_crc >>> 8)
                    ^ TABLE[(modbus_crc ^ aByteArray) & 0xff];
        }
        return modbus_crc;
    }
